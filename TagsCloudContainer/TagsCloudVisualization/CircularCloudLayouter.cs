...
public class CircularCloudLayouter : ILayouter
{
    ...
    public CircularCloudLayouter(Point center, Size fieldSize)
    {
        _pointGenerator = new ArchimedeanSpiralPointGenerator(center, Density);
        _prevRectangles = new List<Rectangle>();
        this._center = center;
        this._field = new Rectangle(center.X - fieldSize.Width / 2,
            center.Y - fieldSize.Height / 2,
            fieldSize.Width,
            fieldSize.Height);
    }
    ...
    public Result<Rectangle> PutNextRectangle(Size rectangleSize)
    {
        foreach (var nextRectangleCenter in _pointGenerator.GetNextPoint())
        {
            ...
            if (CheckIsRectOnField(pressedNextRect))
            {
                _prevRectangles.Add(pressedNextRect);
                return Result.Ok<Rectangle>(pressedNextRect);
            }
            return Result.Fail<Rectangle>("Слишком маленький целевой размер изображения, облако не помещается!");
        }
        return Result.Fail<Rectangle>("Слишком маленький целевой размер изображения, облако не помещается!");
    }
    ...
    private bool CheckIsRectOnField(Rectangle rect) => _field.Contains(rect);
}